...
import { InputFrom,Loading,MarkdownEditor,SelectForm } from '../../../components'
...
import { apiPostCreateProduct } from '../../../apis';
...
const CreateProducts = () => {
  ...
  const onSubmit = async (data) => {
    ...
    dispatch(showModalRedux({modalReduxChildren:<Loading/>}))
    const res = await apiPostCreateProduct(formData)
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className='flex gap-3 items-center my-3'>
                ...
                <SelectForm 
                  ...
                  id='brand'
                  lable='Brand'
                  listOption={categories?.find(el => el?._id === watch('category'))?.brand?.map((item)=> ({value:item,text:item}))}
                />
            </div>
            <div className='my-3'>
              <MarkdownEditor
              ...
              <div className='w-full'>
                ...
                <div className="flex items-center justify-center w-full relative"> 
                    <label htmlFor="thumb" className="bg-transparent z-30 flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:bg-[rgba(0,0,0,.2)]">
                        ...
                        <input id="thumb" {...register('thumb',{required: 'Thumb cannot be blank'})} type="file" className="hidden" />
                    </label>
              ...
              <div className='w-full'>
                ...
                <div className="flex items-center justify-center w-full relative"> 
                    <label htmlFor="images" className="bg-transparent z-30 flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:bg-[rgba(0,0,0,.2)]">
                        ...
                        <input id="images" {...register('images',{required: 'Images cannot be blank'})}  type="file" multiple className="hidden" />
                    </label>
                      ...
                      <div className={clsx('absolute inset-0 z-20 flex flex-wrap gap-3 p-4',showRemove?.image ? 'z-40' : '')}>
                        {preview?.images?.map((el,index)=> (       
                            <div key={`CreateProducts-preview-images-${index}`} className='w-[98px] h-[98px] relative'>
                              <img  src={el?.path} alt={`images-${index}`} className='w-full h-full border object-cover'/>
                              {showRemove?.image && <div className='absolute inset-0 bg-[rgba(0,0,0,.2)]'>
                                <span onClick={() => handleRomoveImages(el?.name)} className='absolute top-0 right-0 cursor-pointer p-2 m-1 hover:bg-white rounded-full'><IoMdClose /></span>
                              </div>}
                            </div>
                        ))}   
                      </div>}
